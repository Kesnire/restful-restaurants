...
router.get('/:id', (req, res) => {
  ...
  const starredRestaurant = STARRED_RESTAURANTS.find((starred) => starred.id === id);
...
router.post('/', (req, res) => {
  ...
  const starred = STARRED_RESTAURANTS.find((restaurant) => restaurant.restaurantId === id);
  if(!restaurant){
    res.sendStatus(404);
    return;
  };
  ...
  const newId = uuidv4();
  const newStarredRestarant = {
    id: newId,
    restaurantId: restaurant.id,
    comment: null
  };

  STARRED_RESTAURANTS.push(newStarredRestarant);

  res.status(200).send({
    id: newStarredRestarant.id,
    comment: newStarredRestarant.comment,
    name: restaurant.name
  });
...
router.delete('/:id', (req, res) => {
  ...
  if(newStarredRestaurants.length === STARRED_RESTAURANTS){
    res.sendStatus(404);
    return;
  }
